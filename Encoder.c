...
INT8U scan_encoder(void) {
    ...
    INT8U encoder_value = INVALID_ENCODER_VALUE;  // Default to invalid unless we detect movement
    ...
    if (direction != ENCODER_STEADY && direction != INVALID_ENCODER_VALUE) {
        ...
        if (direction == ENCODER_RIGHT) {
            encoder_value = ENC_RIGHT_INCREMENT;  // Increment encoder value
        } 
...
void encoder_task(void *pvParameters) {
    ...
    // Add encoder input queue to QueueManager
    create_queue(ENCODER_INPUT, 0);
